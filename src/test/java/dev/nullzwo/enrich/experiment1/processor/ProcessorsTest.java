...
public class ProcessorsTest {
  ...
  static void printMappings() {
    ...
    for (String processor : eventMapping.keySet()) {
      ...
      Map<String, Set<String>> partialMappings = eventMapping.get(processor);
      partialMappings.forEach((key, value) -> {
        if (mappings.containsKey(key)) mappings.get(key).addAll(value);
        else mappings.put(key, value);
      });
    }
